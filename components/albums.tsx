...
export default function Albums() {
  ...
  return (
    <IndexSection
      title={"Listen"}
      classNames={
        /* selected !== null ? `bg-zinc-200 ease-in-out duration-500 transform` :  */ `ease-in-out duration-500 transform bg-gradient-to-b from-zinc-100 to-zinc-100 via-zinc-50`
      }
    >
      <div
        data-testid="albums-section"
        className=" w-full flex flex-row flex-wrap items-start justify-evenly"
      >
        {albumsArr.map((i, index) => (
          ...
          >
            <Image
              ...
              className={`shadow-2xl hover:cursor-pointer  ${selected !== null && selected !== index && "bg-zinc-100 opacity-0  ease-in-out duration-500 transform"} self-start ease-in-out duration-500 transform `}
              src={i.src}
              width={250}
              height={250}
              alt={`Album cover of ${i.title} by ${i.artists}`}
              title={i.title}
            />
            <div
              className={`${selected === index ? "opacity-100 " : "opacity-0 "} self-start max-w-[250px] p-1`}
            >
              <p className="flex flex-wrap font-semibold text-base">
                {i.title}
              </p>
              <p className="flex flex-wrap">{i.artists}</p>
              <Link
                href={i.link}
